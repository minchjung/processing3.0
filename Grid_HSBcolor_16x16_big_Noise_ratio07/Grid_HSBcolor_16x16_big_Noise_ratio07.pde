...
final float increm=0.02; // Interpolation* <-- small gets slower
...
void draw(){
  colorMode(HSB,25000,25000,25000,255);
...
void setColor(){
   ...
   for(int i=0; i < rows; i++){
     ...
     for(int j=0; j< cols; j++){
       ...
       float H = map(ranNum,minPress,maxPress,25000,0); // map the data value to the each color set 
       float S = map(ranNum,minPress,maxPress,20000,25000);// set the saturation and brightnessn not from 0 <-- dark and black (not pretty at all)
       float B = map(ranNum,minPress,maxPress,20000,25000);
       float nf = map(ranNum,minPress,maxPress,0.001,1.0);
       //tx+=nf;
       float nH = noise(nf)*H+minPress;
